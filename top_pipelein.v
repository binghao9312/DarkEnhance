...
reg   [7:0]   R_pixel_FIFO           [0:9];
reg   [7:0]   G_pixel_FIFO           [0:9];
reg   [7:0]   B_pixel_FIFO           [0:9];
reg   [9:0]   addrX_FIFO             [0:9];
reg   [9:0]   addrY_FIFO             [0:9];
...
assign  input_pause = (now_state == StopAndPush)? 1'b1 : 1'b0;
assign  isBoundary  = (addr_X == 0 || addr_X == 511 || addr_Y == 0 || addr_Y == 511) ? 1'b1 : 1'b0;
...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        if (now_state == process && enable) begin
            R_pixel_FIFO[0]  <= data_in_R;
            G_pixel_FIFO[0]  <= data_in_G;
            B_pixel_FIFO[0]  <= data_in_B;
            addrX_FIFO[0]    <= addr_X;
            addrY_FIFO[0]    <= addr_Y;
...
always @(*) begin
    case (now_state)
        ...
        process:            next_state = (done)? IDLE :((addr_X == 511)? StopAndPush : process);
        StopAndPush:        next_state = process;
...
always @(posedge rst or posedge clk)begin
    if(rst)begin
        for (k = 0; k < 512; k = k + 1) begin
            R_row3_register[k] <= 8'd0;
            G_row3_register[k] <= 8'd0;
            B_row3_register[k] <= 8'd0;
        end
    end
    else begin
        if(now_state == process)begin
            R_row3_register[addr_X] <= data_in_R; //8'd255 - data_in_R 
            G_row3_register[addr_X] <= data_in_G; //8'd255 - data_in_G 
            B_row3_register[addr_X] <= data_in_B; //8'd255 - data_in_B               
        end
        else begin
            R_row3_register[addr_X] <= R_row3_register[addr_X]; 
            G_row3_register[addr_X] <= G_row3_register[addr_X];
            B_row3_register[addr_X] <= B_row3_register[addr_X];
        end
    end
end
...
always @(posedge rst or posedge clk)begin
    if(rst)begin
        ...
        if(now_state == StopAndPush)begin
            for (k = 0; k < 512; k = k + 1) begin
...
//=========================== MASKING ===========================
always @(*)begin
    Rm0 = R_row0_register[addr_X - 1];
    Rm1 = R_row0_register[addr_X    ];
    Rm2 = R_row0_register[addr_X + 1];
    Rm3 = R_row1_register[addr_X - 1];
    Rm4 = R_row1_register[addr_X    ];
    Rm5 = R_row1_register[addr_X + 1];
    Rm6 = R_row2_register[addr_X - 1];
    Rm7 = R_row2_register[addr_X    ];
    Rm8 = R_row2_register[addr_X + 1];

    Gm0 = G_row0_register[addr_X - 1];
    Gm1 = G_row0_register[addr_X    ];
    Gm2 = G_row0_register[addr_X + 1];
    Gm3 = G_row1_register[addr_X - 1];
    Gm4 = G_row1_register[addr_X    ];
    Gm5 = G_row1_register[addr_X + 1];
    Gm6 = G_row2_register[addr_X - 1];
    Gm7 = G_row2_register[addr_X    ];
    Gm8 = G_row2_register[addr_X + 1];

    Bm0 = B_row0_register[addr_X - 1];
    Bm1 = B_row0_register[addr_X    ];
    Bm2 = B_row0_register[addr_X + 1];
    Bm3 = B_row1_register[addr_X - 1];
    Bm4 = B_row1_register[addr_X    ];
    Bm5 = B_row1_register[addr_X + 1];
    Bm6 = B_row2_register[addr_X - 1];
    Bm7 = B_row2_register[addr_X    ];
    Bm8 = B_row2_register[addr_X + 1];
...
always @(posedge clk)begin
    pixel_R <= R_pixel_FIFO[9];
    pixel_G <= G_pixel_FIFO[9];
    pixel_B <= B_pixel_FIFO[9];
end
        ...
        if (now_state == process && enable) begin
            ...
            if(isBoundary) begin
                if(addr_Y == 0)begin
                    ...
                    data_out_G <= G_boundary_reg_top[addr_X];
                    data_out_B <= B_boundary_reg_top[addr_X];
                end
                else if(addr_X == 0)begin
                    data_out_R <= R_boundary_reg_left[addr_Y];
                    data_out_G <= G_boundary_reg_left[addr_Y];
                    data_out_B <= B_boundary_reg_left[addr_Y];
                end
                else if(addr_X == 511)begin
                    data_out_R <= R_boundary_reg_right[addr_Y];
                    data_out_G <= G_boundary_reg_right[addr_Y];
                    data_out_B <= B_boundary_reg_right[addr_Y];
                end
                else if(addr_Y == 511)begin
                    data_out_R <= R_boundary_reg_right[addr_X];
                    data_out_G <= G_boundary_reg_right[addr_X];
                    data_out_B <= B_boundary_reg_right[addr_X];
                end                 
            end
            else begin
